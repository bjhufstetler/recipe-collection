...
test('recipe names from state appears in an unordered list', async () => {
  const app = render(<App />);

  // Add the submit button to your setup method:
  const newRecipes = [
    {
      recipeName: 'Lean Pockets',
      recipeInstructions: 'heat in toaster'
    },
    {
      recipeName: 'Hot Pockets',
      recipeInstructions: 'heat in hotter toaster'
    },
    {
      recipeName: 'Hottest Pockets',
      recipeInstructions: 'heat in oven'
    }
  ]
  
  for(let item in newRecipes) {
    userEvent.click(app.getByText('Add Recipe'));
    const nameInput = app.getByLabelText('Recipe name:')
    const instructionsInput = app.getByLabelText('Instructions:')
    const submitButton = app.getByRole('button')
    console.log('logging ', newRecipes[item])
    await userEvent.type(instructionsInput, newRecipes[item].recipeInstructions);
    await userEvent.type(nameInput, newRecipes[item].recipeName);
    await userEvent.click(submitButton)
    //expect(screen.getByRole('listitem')).toBeInTheDocument()
    expect(screen.getByText(newRecipes[item].recipeName)).toBeInTheDocument()
  }
